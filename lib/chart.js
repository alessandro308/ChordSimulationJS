...
var data = [];
for (var _i = 0; _i < proprieties.nodesNumber; _i++) {
    data[chord.nodes[_i].id] = { id: chord.nodes[_i].id, count: 0 };
}

for (var _i2 = 0; _i2 < statistics.length; _i2++) {
    var steps = statistics[_i2].steps;
...
var categories1 = [];
var series1 = [];
for (var _i3 = 0; _i3 < chord.nodes.length; _i3++) {
    var elem = data[chord.nodes[_i3].id];
    categories1.push(elem.id);
    series1.push(elem.count);
}

var distance = [];
for (var _i4 = 1; _i4 < chord.nodes.length; _i4++) {
    distance[_i4] = chord.nodes[_i4].id - chord.nodes[_i4 - 1].id;
}
distance[0] = Math.pow(2, proprieties.addressSize) - lastElement(chord.nodes).id + chord.nodes[0].id;
