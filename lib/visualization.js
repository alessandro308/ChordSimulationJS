...
function drawFingerprint(_node, color) {
	var a = map(_node.id, 0, maxId, 0, TWO_PI) - HALF_PI;
	var x = 300 * cos(a);
	var y = 300 * sin(a);
	var fingerIds = removeDuplicates(_node.finger);

	for (var i = 0; i < fingerIds.length; i++) {
		var a1 = map(fingerIds[i], 0, maxId, 0, TWO_PI) - HALF_PI;
		var x1 = 300 * cos(a1);
		var y1 = 300 * sin(a1);
		stroke(color);
		line(x, y, x1, y1);
...
function setup() {
	...
	for (var i = 0; i < chord.nodesNumber; i++) {
		var a = map(chord.nodes[i].id, 0, maxId, 0, TWO_PI) - HALF_PI;
		var x = 300 * cos(a);
		var y = 300 * sin(a);
		textSize(12);
		fill(0, 102, 153);
...
function draw() {
	background(0xf0f0f0);
	for (var i = 0; i < chord.nodesNumber; i++) {
		var _a = map(chord.nodes[i].id, 0, maxId, 0, TWO_PI) - HALF_PI;
		var _x = 300 * cos(_a);
		var _y = 300 * sin(_a);
		if (showText) {
			textSize(12);
	...
	var val = drawid;
	var a = map(val, 0, maxId, 0, TWO_PI) - HALF_PI;
	var x = 300 * cos(a);
	var y = 300 * sin(a);
	fill(color(255, 0, 0));
	ellipse(x, y, 5, 5);
